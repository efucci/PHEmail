...
import socket, ssl, re, base64, threading, imaplib
...
class Connection:
    ...
    def __init__(self, socket, key, verbose = True):
        self.verbose = verbose
        self.key = key
    ...
    def transmit_fetch(self, mail):
        ...
        self.listen_server(server_tag,mail)
                
    def listen_server(self, server_tag, mail=None):
        ...
        while True:
            ...
            if mail is not None:
                if re.search(r'BODYSTRUCTURE', response) or re.search(r'BODY', response) or re.search(r'HEADER', response) and header is not None:
                    ...
                    if response is not ')':
                        plus = '\nEncrypted-version:\n'
                    self.send_to_client(plus+response)
 ...
 ############ separare le richieste dell'header da quelle del body! #####################
    ...
    def connect_server(self, username, password):
        ...
        self.private_key = read_dk(username, password)
